...
| 09 | `docs/09_task_contracts/` | One work order per module (TC-01…TC-14), plus **TC-15** (researcher-authored M5 C4 remediation: production pipeline wiring), **TC-16** (researcher-authored M6 live-run enablement: the live-provider execution path), and **TC-18** (researcher-authored M6 ingest-crosswalk remediation: Qualtrics multi-header fix): scope, inputs/outputs, acceptance criteria, test requirements, dependencies. The unit Codex issues and signs off against. | ✅ |
